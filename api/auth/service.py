...
async def current_principal(token:str=Depends(oauth2_scheme))->dict:
    ...
    async with engine.connect() as conn:
        row=(await conn.execute(text("SELECT u.id,u.username,u.customer_id,u.is_active,COALESCE(array_agg(DISTINCT p.code) FILTER (WHERE p.code IS NOT NULL),'{}') AS permissions FROM users u LEFT JOIN user_roles ur ON ur.user_id=u.id LEFT JOIN role_permissions rp ON rp.role_id=ur.role_id LEFT JOIN permissions p ON p.id=rp.permission_id WHERE u.id=:id GROUP BY u.id"),{"id":user_id})).first()
