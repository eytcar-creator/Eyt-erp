...
class LoginRequest(BaseModel): username:str=Field(min_length=1,max_length=120); password:str=Field(min_length=8,max_length=200)
class RefreshRequest(BaseModel): refresh_token:str=Field(min_length=40,max_length=300)
class BootstrapRequest(BaseModel): username:str=Field(min_length=3,max_length=120); email:str|None=None; password:str=Field(min_length=12,max_length=200)
@router.post("/login")
async def login(payload:LoginRequest,request:Request):
    ...
    async with engine.begin() as conn:
        ...
        if not row or not row.is_active or not verify_password(payload.password,row.password_hash): raise HTTPException(401,"Invalid credentials")
...
async def refresh(payload:RefreshRequest,request:Request):
    ...
    async with engine.begin() as conn:
        ...
        new=create_refresh_token(); new_id=(await conn.execute(text("INSERT INTO refresh_tokens(user_id,token_hash,expires_at) VALUES(:user,:hash,:exp) RETURNING id"),{"user":row.user_id,"hash":refresh_hash(new),"exp":datetime.now(timezone.utc)+timedelta(days=REFRESH_DAYS)})).scalar_one()
    ...
    return {"access_token":create_access_token(row.user_id),"token_type":"bearer","expires_in":ACCESS_MINUTES*60,"refresh_token":new}
@router.post("/logout")
async def logout(payload:RefreshRequest,request:Request,principal:dict=Depends(current_principal)):
    async with engine.begin() as conn: await conn.execute(text("UPDATE refresh_tokens SET revoked_at=now() WHERE token_hash=:hash AND user_id=:user AND revoked_at IS NULL"),{"hash":refresh_hash(payload.refresh_token),"user":principal["id"]})
    await audit(request,principal,"auth.logout","user",principal["id"]); return {"status":"logged_out"}
@router.get("/me")
async def me(principal:dict=Depends(current_principal)): return {"id":principal["id"],"username":principal["username"],"customer_id":principal["customer_id"],"permissions":principal["permissions"]}
@router.post("/bootstrap",status_code=201)
async def bootstrap(payload:BootstrapRequest,request:Request):
    if request.headers.get("X-Bootstrap-Secret")!=os.getenv("BOOTSTRAP_SECRET"): raise HTTPException(403,"Bootstrap is disabled or unauthorized")
    if engine is None: raise HTTPException(503,"Database is not configured")
    async with engine.begin() as conn:
