...
from .service import (ACCESS_MINUTES, REFRESH_DAYS, audit, create_access_token,
                       create_refresh_token, current_principal, hash_password,
                       refresh_hash, verify_password)
...
async def login(payload: LoginRequest, request: Request):
    ...
    async with engine.begin() as conn:
        ...
        principal = {"id": row.id}
    await audit(request, principal, "auth.login", "user", row.id)
...
async def refresh(payload: RefreshRequest, request: Request):
    ...
    async with engine.begin() as conn:
        row = (await conn.execute(text("""
            SELECT rt.id, rt.user_id, u.is_active FROM refresh_tokens rt
        ...
        new_id = (await conn.execute(text("""
            ...
            VALUES (:user, :hash, :exp) RETURNING id
        """), {"user": row.user_id, "hash": refresh_hash(new_refresh), "exp": datetime.now(timezone.utc) + timedelta(days=REFRESH_DAYS)})).scalar_one()
        await conn.execute(text("UPDATE refresh_tokens SET revoked_at = now(), replaced_by_id = :new WHERE id = :old"), {"new": new_id, "old": row.id})
    principal = {"id": row.user_id}
    await audit(request, principal, "auth.refresh", "refresh_token", row.id)
...
async def logout(payload: RefreshRequest, request: Request, principal: dict = Depends(current_principal)):
    ...
    await audit(request, principal, "auth.logout", "user", principal["id"])
...
async def bootstrap(payload: BootstrapRequest, request: Request):
    """One-time first-admin bootstrap. Requires BOOTSTRAP_SECRET and an empty user table."""
    import os
    ...
    if not expected or not supplied or supplied != expected:
        raise HTTPException(status_code=403, detail="Bootstrap is disabled or unauthorized")
    if engine is None:
        raise HTTPException(503, "Database is not configured")
    async with engine.begin() as conn:
        ...
        """), {"username": payload.username, "email": payload.email, "hash": hash_password(payload.password)})).scalar_one()
    ...
    await audit(request, {"id": user_id}, "auth.bootstrap", "user", user_id)
    return {"id": user_id, "username": payload.username, "role": "CEO"}
