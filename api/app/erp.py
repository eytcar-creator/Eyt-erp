...
class ProductCreate(BaseModel):
    sku:str=Field(min_length=1,max_length=100); name_fa:str=Field(min_length=1,max_length=250); category_id:UUID|None=None; unit:str=Field(default="PCS",max_length=30); barcode:str|None=Field(default=None,max_length=100); oem_code:str|None=Field(default=None,max_length=120); brand:str|None=Field(default=None,max_length=120); material:str|None=Field(default=None,max_length=120); weight_kg:Decimal|None=Field(default=None,ge=0); cost_price:Decimal=Field(default=Decimal("0"),ge=0); sale_price:Decimal=Field(default=Decimal("0"),ge=0); min_stock:Decimal=Field(default=Decimal("0"),ge=0); is_sellable:bool=True; is_purchasable:bool=True; image_url:str|None=None
...
@router.get("/products")
async def list_products(q:str|None=Query(default=None,max_length=100),active_only:bool=True,limit:int=Query(default=50,ge=1,le=200),offset:int=Query(default=0,ge=0),principal:dict=Depends(require_permission("catalog.read"))):
    ...
    async with engine.connect() as conn:
        result=await conn.execute(text(f"SELECT {PRODUCT_COLUMNS},c.name_fa AS category_name FROM products p LEFT JOIN product_categories c ON c.id=p.category_id WHERE {' AND '.join(filters) or 'TRUE'} ORDER BY p.sku LIMIT :limit OFFSET :offset"),params); return [dict(r._mapping) for r in result]
...
async def get_product(product_id:UUID,principal:dict=Depends(require_permission("catalog.read"))):
    await require_engine()
    async with engine.connect() as conn: row=(await conn.execute(text(f"SELECT {PRODUCT_COLUMNS},c.name_fa AS category_name FROM products p LEFT JOIN product_categories c ON c.id=p.category_id WHERE p.id=:id"),{"id":product_id})).first()
...
async def inventory(product_id:UUID|None=None,warehouse_id:UUID|None=None,principal:dict=Depends(require_permission("inventory.read"))):
    ...
    async with engine.connect() as conn:
        result=await conn.execute(text(f"SELECT ib.product_id,p.sku,p.name_fa,ib.warehouse_id,w.code AS warehouse_code,w.name_fa AS warehouse_name,ib.on_hand_qty,ib.reserved_qty,(ib.on_hand_qty-ib.reserved_qty) AS available_qty,ib.reorder_point,ib.updated_at FROM inventory_balances ib JOIN products p ON p.id=ib.product_id JOIN warehouses w ON w.id=ib.warehouse_id WHERE {' AND '.join(filters) or 'TRUE'} ORDER BY p.sku,w.code"),params); return [dict(r._mapping) for r in result]
...
async def post_stock_transaction(payload:StockTransaction,request:Request,principal:dict=Depends(require_permission("inventory.adjust"))):
    ...
    async with engine.begin() as conn:
        ...
        row=(await conn.execute(text("SELECT on_hand_qty,reserved_qty FROM inventory_balances WHERE product_id=:product_id AND warehouse_id=:warehouse_id FOR UPDATE"),{"product_id":payload.product_id,"warehouse_id":payload.warehouse_id})).first(); on_hand,reserved=(Decimal(row.on_hand_qty),Decimal(row.reserved_qty)) if row else (Decimal("0"),Decimal("0")); new_on_hand=on_hand+delta
        if new_on_hand<reserved or new_on_hand<0: raise HTTPException(409,"Insufficient available stock")
        if row: await conn.execute(text("UPDATE inventory_balances SET on_hand_qty=:qty,updated_at=now() WHERE product_id=:product_id AND warehouse_id=:warehouse_id"),{"qty":new_on_hand,"product_id":payload.product_id,"warehouse_id":payload.warehouse_id})
        else: await conn.execute(text("INSERT INTO inventory_balances(product_id,warehouse_id,on_hand_qty) VALUES(:product_id,:warehouse_id,:qty)"),{"product_id":payload.product_id,"warehouse_id":payload.warehouse_id,"qty":new_on_hand})
...
async def dashboard_summary(principal:dict=Depends(require_permission("reporting.read"))):
    await require_engine()
    async with engine.connect() as conn:
        result=await conn.execute(text("SELECT (SELECT count(*) FROM products WHERE is_active=true) AS active_products,(SELECT count(*) FROM customers WHERE is_active=true) AS active_customers,(SELECT count(*) FROM suppliers WHERE is_active=true) AS active_suppliers,(SELECT coalesce(sum(on_hand_qty),0) FROM inventory_balances) AS total_units_on_hand,(SELECT count(*) FROM sales_orders WHERE status IN ('DRAFT','CONFIRMED','RESERVED','PARTIAL')) AS open_sales_orders,(SELECT count(*) FROM production_orders WHERE status IN ('PLANNED','RELEASED','IN_PROGRESS')) AS active_production_orders")); return dict(result.first()._mapping)
