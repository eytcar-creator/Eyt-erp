...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column("request_no", sa.String(80), nullable=False, unique=True),
        ...
        sa.CheckConstraint("status IN ('DRAFT','SUBMITTED','APPROVED','REJECTED','ORDERED','CANCELLED')", name="ck_purchase_request_status"),
    )
    op.create_table(
        "purchase_request_items",
        sa.Column("id", UUID(as_uuid=True), primary_key=True, server_default=sa.text("gen_random_uuid()")),
        sa.Column("request_id", UUID(as_uuid=True), sa.ForeignKey("purchase_requests.id", ondelete="CASCADE"), nullable=False),
