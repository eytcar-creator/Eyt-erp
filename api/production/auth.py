...
PERMISSIONS = {
    ...
    "procurement.receive", "crm.read", "crm.write", "automation.read", "automation.write",
}
...
class BootstrapInput(BaseModel):
    username: str = Field(min_length=3, max_length=120); password: str = Field(min_length=12, max_length=200); email: str|None = Field(default=None,max_length=250)
def db_connection():
...
def secret():
    ...
    if not value: raise RuntimeError("JWT_SECRET must be configured")
    return value
def hash_password(password):
    salt=secrets.token_bytes(16); digest=hashlib.scrypt(password.encode(),salt=salt,n=2**14,r=8,p=1); return f"scrypt$16384$8$1${salt.hex()}${digest.hex()}"
def verify_password(password,encoded):
    try:
        scheme,n,r,p,salt_hex,digest_hex=encoded.split("$")
        if scheme!="scrypt": return False
        actual=hashlib.scrypt(password.encode(),salt=bytes.fromhex(salt_hex),n=int(n),r=int(r),p=int(p)); return hmac.compare_digest(actual.hex(),digest_hex)
    except (ValueError,TypeError): return False
def access_token(user_id):
    now=datetime.now(timezone.utc); return jwt.encode({"sub":str(user_id),"type":"access","iat":now,"exp":now+timedelta(minutes=ACCESS_MINUTES)},secret(),algorithm=ALGORITHM)
def refresh_token(): return secrets.token_urlsafe(64)
def refresh_hash(token): return hashlib.sha256(token.encode()).hexdigest()
def current_principal(token:str=Depends(oauth2_scheme)):
    try:
        payload=jwt.decode(token,secret(),algorithms=[ALGORITHM]);
        if payload.get("type")!="access": raise ValueError
        user_id=UUID(payload["sub"])
    except (jwt.InvalidTokenError,ValueError,KeyError): raise HTTPException(status.HTTP_401_UNAUTHORIZED,"Invalid or expired access token",headers={"WWW-Authenticate":"Bearer"})
...
def require_permission(code:str)->Callable:
    def dependency(principal:dict=Depends(current_principal)):
        if code not in principal["permissions"]: raise HTTPException(403,"Insufficient permission")
        return principal
    return dependency
def _audit_entity_id(conn,entity_id):
    # eyt_audit_logs.entity_id is UUID-only. Legacy numeric IDs and
    # business document numbers must never be written into this column.
...
def bootstrap(payload:BootstrapInput,request:Request):
    ...
    with db_connection() as conn:
        ...
        role_id=conn.execute("SELECT id FROM eyt_roles WHERE name='CEO'").fetchone()[0]; user_id=conn.execute("INSERT INTO eyt_users(username,email,password_hash) VALUES(%s,%s,%s) RETURNING id",(payload.username,payload.email,hash_password(payload.password))).fetchone()[0]; conn.execute("INSERT INTO eyt_user_roles(user_id,role_id) VALUES(%s,%s)",(user_id,role_id)); conn.commit()
...
def login(payload:LoginInput,request:Request):
    with db_connection() as conn:
        ...
        token=refresh_token(); now=datetime.now(timezone.utc); conn.execute("UPDATE eyt_users SET last_login_at=%s WHERE id=%s",(now,row[0])); conn.execute("INSERT INTO eyt_refresh_tokens(user_id,token_hash,expires_at) VALUES(%s,%s,%s)",(row[0],refresh_hash(token),now+timedelta(days=REFRESH_DAYS))); conn.commit()
...
def refresh(payload:RefreshInput,request:Request):
    with db_connection() as conn:
        ...
        new=refresh_token(); new_id=conn.execute("INSERT INTO eyt_refresh_tokens(user_id,token_hash,expires_at) VALUES(%s,%s,%s) RETURNING id",(row[1],refresh_hash(new),datetime.now(timezone.utc)+timedelta(days=REFRESH_DAYS))).fetchone()[0]; conn.execute("UPDATE eyt_refresh_tokens SET revoked_at=now(),replaced_by_id=%s WHERE id=%s",(new_id,row[0])); conn.commit()
...
def logout(payload:RefreshInput,request:Request,principal:dict=Depends(current_principal)):
    with db_connection() as conn: conn.execute("UPDATE eyt_refresh_tokens SET revoked_at=now() WHERE token_hash=%s AND user_id=%s AND revoked_at IS NULL",(refresh_hash(payload.refresh_token),principal["id"])); conn.commit()
    audit(request,principal,"auth.logout",principal["id"]); return {"status":"logged_out"}
@router.get("/me")
def me(principal:dict=Depends(current_principal)): return principal
